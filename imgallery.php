...
class ImgGallery {
  ...
  public function __construct(
  	...
  	$thumbsize=150, //Change this to match your thumbnail size
  	$maxsize=640, //Change this to match your maximum image size
  	$folderpath="images/", //Change this to match your folder path
  	$elements=array()
  ){
  ...
  public function getImageArray(){
  	//looks for images inside this folder
   ...
   public function display($showit=1){
    ...
    <div id="easyimgallery">
      ...
      if(!empty($this->elements)){foreach($this->elements as $img){
		...
		//end sets the internal pointer of an array to its end
        $imgname=end(explode("/",$img));
        $markup.='<li><a href="i'.$maxsize.'" class="lightbox" title="'.$imgname.'">
          <img src="i'.$thumb.'" alt="'.$imgname.'" />
        </a></li>';
      }}
      $markup.='
      </ul>
  ...
  //class intance
	...
	$gallery->folderpath="images/".$_SESSION['username'];
  	$gallery->loadImages();
  	$gallery->display();
  }
  ...
  public function getMaxImage($src){
	...
	else //Create the image
	{
		...
		//saving dynamically created pics
